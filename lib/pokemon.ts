...
export async function getPokemonList({
  ...
  const result = await fetch("http://localhost:8080/api/pokemon");
  const pokemonListJson = await result.json();
...
export async function getPokemonItem(idOrName: number | string) {
  const result = await fetch(`http://localhost:8080/api/pokemon/${idOrName}`);
